...
//pages/TransactionHistory.jsx 
// MOBILE RESPONSIVE VERSION - User Transaction History Page
import React, { useState, useEffect } from 'react';
import { 
  CreditCard, 
  ArrowUpRight, 
...
const TransactionHistory = () => {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 max-w-6xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-[#7042D2] to-[#946BEF] rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 text-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-white bg-opacity-10 rounded-full transform translate-x-12 -translate-y-12"></div>
        <div className="absolute bottom-0 left-0 w-24 h-24 bg-white bg-opacity-5 rounded-full transform -translate-x-8 translate-y-8"></div>
        ...
        <div className="relative z-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
            ...
            <div className="bg-white bg-opacity-20 p-3 rounded-full self-start">
              <CreditCard className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
          </div>
          
          <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 text-sm opacity-90">
            <span>Available for bookings</span>
            <button 
              onClick={handleRefresh}
              disabled={refreshing}
              className="flex items-center space-x-1 hover:opacity-75 transition-opacity self-start"
            >
              <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
              <span>Refresh</span>
            </button>
                ...
                return (
                  <div key={transaction.id} className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        {getTransactionIcon(transaction.type)}
                        <span className={`text-base sm:text-lg font-semibold ${amountStyle.color}`}>
                          {amountStyle.amount}
                        </span>
                      </div>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${statusStyle.bg} ${statusStyle.text}`}>
      ...
      {walletData.transactions && walletData.transactions.length > 0 && (
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 sm:col-span-2 lg:col-span-1">
            <div className="flex items-center">

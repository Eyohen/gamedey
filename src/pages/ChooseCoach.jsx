...
//   // Get next 7 days with availability status
...
//   useEffect(() => {
//     const currentSelectedDate = new Date(calendarDate);
//     currentSelectedDate.setDate(selectedDate);

//     if (coachId) {
//       fetchAvailableSlots(currentSelectedDate);
//     }
//   }, [selectedDate, calendarDate, coachId]);
...
//       // Don't show error for this, just continue with default behavior
//     }
//   };
...
//     const currentSelectedDate = new Date(calendarDate);
//     currentSelectedDate.setDate(selectedDate);
//     const timeSlotDate = convertTimeToDate(timeSlot, currentSelectedDate);
...
//       const date = new Date(calendarDate.getFullYear(), calendarDate.getMonth() - 1, day);
//       days.push({
//         day,
//         date,
//         isCurrentMonth: false,
...
//     // if (dayObj.isCurrentMonth && !dayObj.isPast && !dayObj.isUnavailable) {
//     //   const newDate = new Date(calendarDate.getFullYear(), calendarDate.getMonth(), dayObj.day);
//     //   setCalendarDate(newDate);
...
//                     }}
//                     disabled={isDisabled}
//                     className={`px-3 sm:px-4 py-2 sm:py-3 rounded-lg text-sm font-medium ${selectedDay === day.day && selectedDate === day.date
//                       ? 'bg-purple-500 text-white'
//                       : isDisabled
...
//                   className={`py-2 px-2 sm:px-3 rounded-lg text-xs sm:text-sm font-medium border ${selectedTimeSlot === time
//                     ? 'bg-gray-900 text-white border-gray-900'
//                     : isAvailable
//                       ? 'bg-white text-gray-700 border-gray-300 hover:border-purple-300'
//                       : 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed'
...
//                     const currentSelectedDate = new Date(calendarDate);
//                     currentSelectedDate.setDate(selectedDate);
//                     return convertTimeToDate(time, currentSelectedDate) > convertTimeToDate(selectedTimeSlot, currentSelectedDate);
//                   })
//                   .map((time, index) => (
//                     <option key={index} value={time}>
//                       {time}
...
//                 const isSelected = selectedDate === dayObj.day && calendarDate.getMonth() === dayObj.date.getMonth();
            ...
            {coach.galleryImages && coach.galleryImages.length > 0 ? (
              coach.galleryImages.slice(0, 2).map((image, index) => (
                <div key={index} className="w-20 sm:w-24 h-12 sm:h-16 bg-gradient-to-br from-green-400 to-blue-500 rounded-lg relative overflow-hidden">
                  <img 
                  src={image} 
                  alt={`${coach.User?.firstName} coaching`} className="w-full h-full object-cover" />
                </div>
              ))
            ) : (
              <>

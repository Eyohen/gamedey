  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        // Route based on detected user role
        const getUserRole = (userData) => {
          console.log('Detecting role for user:', userData);
          if (userData.Coach || userData.role === 'coach') {
            console.log('Detected role: coach');
            return 'coach';
          }
          if (userData.ownedFacilities?.length > 0 || userData.Facility || userData.role === 'facility' || userData.role === 'facility-owner') {
        ...
        // Update auth context with role (this will auto-detect role)
        login(user);

        console.log('Login successful, navigating to dashboard for role:', detectedRole);

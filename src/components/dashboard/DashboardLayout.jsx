...
const MenuItem = ({ icon, title, path, collapsed, active, onClick, isMobile, onMobileItemClick }) => {
  ...
  if (onClick) {
    return (
      <button
        onClick={handleClick}
        className={`flex items-center w-full px-4 py-3 text-left rounded-xl text-gray-500 hover:bg-gray-50 hover:text-black font-semibold transition-colors ${collapsed && !isMobile ? 'justify-center px-2' : ''
          }`}
      >
  ...
  return (
    <Link
      to={path}
      onClick={handleClick}
      className={`flex items-center px-3 py-3 text-left rounded-xl transition-colors ${collapsed && !isMobile ? 'justify-center w-auto' : 'w-[200px]'
        } ${active
